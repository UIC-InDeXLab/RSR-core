/*
 * RSR CUDA kernel v5.8
 *
 * Main changes vs v5.6:
 *   1. Same packed group metadata path.
 *   2. Thread dispatch uses 1024 threads for k > 4.
 *
 * Intended to trade occupancy for more group-level parallelism on larger k.
 */
...
__device__ __forceinline__ void process_group(
    ...
    for (; i + 96 < len; i += 128) {
        ...
        #pragma unroll 4
        for (int u = 0; u < 4; ++u) {
            s += __ldg(&v[__ldg(&perm[start + i + u * 32])]);
        }
        local_sum += s;
    }
    for (; i < len; i += 32) {
        local_sum += __ldg(&v[__ldg(&perm[start + i])]);
    }
...
__global__ void rsr_gemv_v5_8_kernel(
    const int32_t* __restrict__ perms,
    const int4* __restrict__ group_packed,
...
void rsr_gemv_v5_8(
    ...
    if (k <= 4) {
        ...
        rsr_gemv_v5_8_kernel<threads><<<num_blocks, threads, smem_bytes>>>(
            perms.data_ptr<int32_t>(),
            gp4,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("rsr_gemv_v5_8", &rsr_gemv_v5_8, "RSR GEMV CUDA v5.8");
