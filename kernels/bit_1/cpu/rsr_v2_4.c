/*
 * RSR GEMV kernel v2.4 — Targeted cache optimizations:
 *   1. schedule(static) instead of dynamic: eliminates false sharing on output
 *      (consecutive blocks → consecutive output → same thread → same cache lines)
 *      Also reduces OpenMP scheduling overhead for many blocks.
 *   2. Pre-allocated, cache-line aligned per-thread v_perm buffers
 *   3. Same gather + contiguous aggregation strategy as v2
 */
...
void rsr_gemv_v2_4(
    ...
    /* Pre-allocate aligned v_perm buffers — one per thread */
    float **v_perm_bufs = (float **)malloc((size_t)num_threads * sizeof(float *));
    for (int t = 0; t < num_threads; t++) {
        v_perm_bufs[t] = (float *)aligned_alloc(64, ((size_t)n * sizeof(float) + 63) & ~63UL);
    }

#ifdef _OPENMP
    /* static scheduling: consecutive blocks go to same thread,
       so their output writes (out[b*k .. (b+1)*k-1]) are contiguous
    ...
    for (int b = 0; b < num_blocks; b++) {
        ...
        /* Phase 1: Gather — pre-permute v into contiguous buffer */
        int i = 0;
#if defined(__AVX2__)
        for (; i + 15 < n; i += 16) {
            if (i + 64 < n) {
        ...
        /* Phase 2+3: Aggregate contiguous v_perm slices, then scatter */
        int start = 0;
