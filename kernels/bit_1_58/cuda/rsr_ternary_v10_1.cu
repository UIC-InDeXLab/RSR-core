/*
 * RSR ternary CUDA kernel v10.1 — v6.5 with a 64-column inner loop.
 *
 * Doubles the useful work per loop iteration relative to v6.5 by consuming
 * four int4 activation vectors and four packed-weight dwords at a time.
 */
...
__global__ void quantize_fp32_to_int8_v101(
...
__device__ __forceinline__ int decode_byte_v101(uint8_t b) {
    int wp = (b & 3)
           | (((b >> 2) & 3) << 8)
           | (((b >> 4) & 3) << 16)
           | (((b >> 6) & 3) << 24);
...
rsr_direct_v10_1_kernel(
    ...
    const int stride64 = K_DIM * 64;
    const int stride32 = K_DIM * 32;
    const int n64 = (n / stride64) * stride64;
    const int n32 = ((n - n64) / stride32) * stride32 + n64;

    for (int blk = blockIdx.x; blk < num_blocks; blk += gridDim.x) {
        const uint8_t* w_row = w + (static_cast<int64_t>(blk) * K + row) * (n / 4);
        int32_t acc = 0;

        for (int base = tx * 64; base < n64; base += stride64) {
            int4 va0 = *reinterpret_cast<const int4*>(v_i8 + base);
            int4 va1 = *reinterpret_cast<const int4*>(v_i8 + base + 16);
            int4 va2 = *reinterpret_cast<const int4*>(v_i8 + base + 32);
            int4 va3 = *reinterpret_cast<const int4*>(v_i8 + base + 48);
            ...
            const uint32_t wb2 = __ldg(reinterpret_cast<const uint32_t*>(w_row + base / 4 + 8));
            const uint32_t wb3 = __ldg(reinterpret_cast<const uint32_t*>(w_row + base / 4 + 12));

            #pragma unroll
            for (int g = 0; g < 4; g++)
                acc = __dp4a(*(reinterpret_cast<const int*>(&va0) + g),
                             decode_byte_v101(static_cast<uint8_t>(wb0 >> (g * 8))), acc);
            #pragma unroll
            for (int g = 0; g < 4; g++)
                acc = __dp4a(*(reinterpret_cast<const int*>(&va1) + g),
                             decode_byte_v101(static_cast<uint8_t>(wb1 >> (g * 8))), acc);
            #pragma unroll
            for (int g = 0; g < 4; g++)
                acc = __dp4a(*(reinterpret_cast<const int*>(&va2) + g),
                             decode_byte_v101(static_cast<uint8_t>(wb2 >> (g * 8))), acc);
            #pragma unroll
            for (int g = 0; g < 4; g++)
                acc = __dp4a(*(reinterpret_cast<const int*>(&va3) + g),
                             decode_byte_v101(static_cast<uint8_t>(wb3 >> (g * 8))), acc);
        }

        for (int base = n64 + tx * 32; base < n32; base += stride32) {
            int4 va0 = *reinterpret_cast<const int4*>(v_i8 + base);
            int4 va1 = *reinterpret_cast<const int4*>(v_i8 + base + 16);
        ...
        for (int base = n32 + tx * 16; base < n; base += K_DIM * 16) {
            ...
            const uint32_t wb = __ldg(reinterpret_cast<const uint32_t*>(w_row + base / 4));

            #pragma unroll
            for (int g = 0; g < 4; g++)
                acc = __dp4a(*(reinterpret_cast<const int*>(&va) + g),
                             decode_byte_v101(static_cast<uint8_t>(wb >> (g * 8))), acc);
        }
        ...
        if (tx == 0)
            out[static_cast<int64_t>(blk) * K + row] = static_cast<float>(acc) * iscale;
    }
}

}  // namespace

static void launch_v10_1(
    const uint8_t* w,
    const int8_t* v_i8,
...
#define LAUNCH(K, KD) \
    rsr_direct_v10_1_kernel<K, KD, K * KD><<<grid_size, K * KD>>>(w, v_i8, inv_scale, out, n, num_blocks)
...
void rsr_direct_v10_1_fused(
    ...
    quantize_fp32_to_int8_v101<<<1, 256>>>(
        v.data_ptr<float>(),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("rsr_direct_v10_1_fused", &rsr_direct_v10_1_fused,
          "RSR direct INT2+INT8+dp4a GEMV v10.1 (64-column inner loop)");
}
