/*
 * RSR ternary CUDA kernel v6.3 — v6.0 with wider loads (K_per_loop=32).
 *
 * Each loop iteration processes 32 columns instead of 16:
 *   - 2× int4 loads  (32 INT8 input values, 32 bytes)
 *   - 1× uint2 load  (8 bytes = 32 INT2 weights, coalesced double-word)
 *   - 8× dp4a calls
 *
 * Halves loop iteration count (16 vs 32 for n=16384, kd=32) reducing:
 *   - Loop overhead / branch prediction cost
 *   - Per-iteration decode: unrolled expand+subtract for 8 bytes
 *
 * Also uses __ldg() for weight reads to place them in the read-only cache.
 */
...
__global__ void quantize_fp32_to_int8_v63(
...
// Decode 1 byte (4 INT2) → int32 (4 INT8 packed)
__device__ __forceinline__ int decode_byte(uint8_t b) {
    int wp = (b & 3)
           | (((b >> 2) & 3) << 8)
           | (((b >> 4) & 3) << 16)
           | (((b >> 6) & 3) << 24);
...
rsr_direct_v6_3_kernel(
    ...
    // Main loop: K_per_loop=32 — each thread processes 32 columns/step
    // stride = K_DIM * 32 columns per warp-step
    const int stride32 = K_DIM * 32;
    const int n32 = (n / stride32) * stride32;

    for (int base = tx * 32; base < n32; base += stride32) {
        // Load 32 INT8 input values (2 × int4)
        int4 va0 = *reinterpret_cast<const int4*>(v_i8 + base);
        int4 va1 = *reinterpret_cast<const int4*>(v_i8 + base + 16);

        // Load 8 bytes = 32 INT2 weight values (1 × uint2, using __ldg)
        const uint8_t* wp = w_row + base / 4;
        const uint32_t wb0 = __ldg(reinterpret_cast<const uint32_t*>(wp));
        const uint32_t wb1 = __ldg(reinterpret_cast<const uint32_t*>(wp + 4));

        // 8 dp4a for 32 columns
        #pragma unroll
        for (int g = 0; g < 4; g++) {
            acc = __dp4a(
                *(reinterpret_cast<const int*>(&va0) + g),
                decode_byte(static_cast<uint8_t>(wb0 >> (g * 8))),
                acc);
        }
        #pragma unroll
        for (int g = 0; g < 4; g++) {
            acc = __dp4a(
                *(reinterpret_cast<const int*>(&va1) + g),
                decode_byte(static_cast<uint8_t>(wb1 >> (g * 8))),
                acc);
        }
    }
    ...
    for (int base = n32 + tx * 16; base < n; base += K_DIM * 16) {
        ...
        for (int g = 0; g < 4; g++)
            acc = __dp4a(*(reinterpret_cast<const int*>(&va) + g),
                         decode_byte(static_cast<uint8_t>(wb >> (g * 8))), acc);
    }
...
#define LAUNCH(K, KD) \
    ...
    if (k_dim == 32) {
        ...
        switch (k) {
            case 2:  LAUNCH(2,  8); break; case 4:  LAUNCH(4,  8); break;
            case 6:  LAUNCH(6,  8); break; case 8:  LAUNCH(8,  8); break;
            case 10: LAUNCH(10, 8); break; case 12: LAUNCH(12, 8); break;
            default: TORCH_CHECK(false, "k not supported");
        }
    } else {
...
void rsr_direct_v6_3_fused(
    ...
    quantize_fp32_to_int8_v63<<<1, 256>>>(
        v.data_ptr<float>(),
    ...
    launch_v6_3(packed_w.data_ptr<uint8_t>(), v_i8_buf.data_ptr<int8_t>(),
                inv_scale_buf.data_ptr<float>(), out.data_ptr<float>(),
                n, k, num_blocks, k_dim);
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("rsr_direct_v6_3_fused", &rsr_direct_v6_3_fused,
          "RSR direct INT2+INT8+dp4a GEMV v6.3 (wider loads, __ldg)");
}
