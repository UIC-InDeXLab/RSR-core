"""
RSR CUDA v5.7 — v5.6 packed metadata kernel + sorted perms within groups.

This combines packed per-group metadata with gather-locality preprocessing.
"""
...
_module = load(
    name="rsr_cuda_v5_6",
    sources=[os.path.join(_kernel_dir, "rsr_v5_6.cu")],
    extra_cuda_cflags=["-O3"],
    verbose=True,  # see the JIT logs
)


class RSRCudaV5_7Multiplier(Multiplier):
    ...
    def prep(self):
        ...
        # Sort perm indices inside each group to improve gather locality.
        ...
        self._perms = torch.from_numpy(perms_cpu.copy()).to(self.device)
    ...
    def __call__(self, v: torch.Tensor) -> torch.Tensor:
        ...
        _module.rsr_gemv_v5_6(  # type: ignore
            self._perms,
            self._group_packed,
            self._block_meta,
            v_gpu,
