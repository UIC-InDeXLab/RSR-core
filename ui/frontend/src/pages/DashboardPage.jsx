  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card><StatCard label="Preprocessed Models" value={models.length} sub={`${cpuModels} CPU / ${cudaModels} CUDA`} to="/models" /></Card>
        <Card><StatCard label="Multipliers" value={multipliers.length} to="/multipliers" /></Card>
        <Card><StatCard label="Total Size" value={`${(totalSize / 1024).toFixed(1)} GB`} sub="preprocessed data" /></Card>
        <Card><StatCard label="CUDA" value={sys?.cuda_available ? "Available" : "N/A"} sub={sys?.cuda_device || "CPU only"} /></Card>
      </div>
