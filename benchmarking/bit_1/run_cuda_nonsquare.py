"""
Benchmark inference time of non-square binary matrix-vector multiply on CUDA.

Compares:
  - pytorch_fp32: dense torch matmul on CUDA (baseline)
  - pytorch_fp16: FP16 matmul on CUDA
  - rsr_nonsquare: RSR v5.9 non-square multiplier
...
from multiplier.bit_1.pytorch import PytorchMultiplier, PytorchFP16Multiplier
from multiplier.bit_1.cuda.rsr_cuda_v5_9_nonsquare import RSRCudaV5_9NonSquareMultiplier
...
def random_binary(n_rows, n_cols):
    return torch.randint(0, 2, (n_rows, n_cols), dtype=torch.float32)
...
def main():
    ...
    labels = ["pytorch_fp32", "pytorch_fp16", "rsr_nonsquare"]
    col_w = 18
    ...
    for n_rows, n_cols in shapes:
        ...
        for k in k_values:
            try:
                m_rsr = RSRCudaV5_9NonSquareMultiplier(M, k)
                t_rsr = bench_inference_cuda(m_rsr, v, warmup=warmup, repeats=repeats)
            except Exception as e:
                print(f"  [error rsr_nonsquare k={k}: {e}]")
                t_rsr = float("nan")
