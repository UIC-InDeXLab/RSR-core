"""
Benchmark CUDA binary (1-bit) multipliers on a given list of matrix shapes.
...
_LABEL_MAP = {
    "rsr_cuda_adaptive": "RSR_adaptive",
    "rsr_cuda_v5_9_nonsquare": "RSR",
}
...
def _stem_to_label(stem):
    ...
    if stem.startswith("rsr_cuda_v"):
        tag = stem[len("rsr_cuda_v"):]
        if tag.endswith("_nonsquare"):
            tag = tag[: -len("_nonsquare")] + "(ns)"
        return "cuda_v" + tag.replace("_", ".")
    return stem
...
def discover_versions():
    ...
    for p in sorted(cuda_dir.glob("*.py")):
        if p.stem in _EXCLUDE or p.stem.startswith("_"):
            continue
        full = f"multiplier.bit_1.cuda.{p.stem}"
