"""
Benchmark inference time of CPU multipliers across n and k values.
...
def discover_all_versions():
    ...
    for stem in stems:
        full = f"multiplier.bit_1.cpu.{stem}"
...
def main():
    ...
    # Split: baselines don't need k (instantiated once per n)
    ...
    for n in n_values:
        print(f"\n{'='*80}")
        print(
            f"  n = {n}   (matrix: {n*n*4 / 1024**2:.1f} MB float32 | {n*n//8//1024} KB 1-bit)"
        )
        print(f"{'='*80}")
        ...
        for k in k_values:
            if n % k == 0:
                ...
                # Only run pytorch, bitnet, and rsr_adaptive
                filtered_base_times = [
                    t if lbl in ("pytorch", "bitnet") else float("nan")
